program amr2cube
  ...
  integer::rhog, rhod, temp, metal, vx, vy, vz, xhi  
  ...
  integer::grp_nb !which summit subfolder 'group_XXXXXX'
  real(KIND=8),dimension(1:8,1:3)::xc
  real(KIND=8),dimension(1:3)::xbound=(/0d0,0d0,0d0/)
#ifdef SIXDIGITS
  character(LEN=6)::nchar,ncharcpu,grp_name
#else
  character(LEN=5)::nchar,ncharcpu,grp_name
#endif
  character(LEN=80)::ordering
  character(LEN=256)::nomfich,repository,outfich,outfich_type,filetype='bin',outnb
...
#ifdef SIXDIGITS
  nchar=repository(ipos+7:ipos+14)
  nomfich=TRIM(repository)//'/group_000001'//'/hydro_'//TRIM(nchar)//'.out000001'
#else
  nchar=repository(ipos+7:ipos+13)
  nomfich=TRIM(repository)//'/group_000001'//'/hydro_'//TRIM(nchar)//'.out00001'
#endif
...
#ifdef SIXDIGITS
  nomfich=TRIM(repository)//'/group_000001'//'/amr_'//TRIM(nchar)//'.out000001'
#else
  nomfich=TRIM(repository)//'/group_000001'//'/amr_'//TRIM(nchar)//'.out00001'
#endif
  inquire(file=nomfich, exist=ok) ! verify input file
  if ( .not. ok ) then
     print *,TRIM(nomfich)//' not found.'
...
#ifdef SIXDIGITS  
  nomfich=TRIM(repository)//'/group_000001/info_'//TRIM(nchar)//'.txt'
#else
  ...
  do ilevel=1,lmax
     ...
     allocate(grid(ilevel)%cube(imin:imax,jmin:jmax,kmin:kmax,8))
  ...
  do k=1,ncpu_read
     ...
     grp_nb=int((icpu-1.)/8192.)+1
     call title(grp_nb,grp_name)
     
     ! Open AMR file and skip header
     nomfich=TRIM(repository)//'/group_'//TRIM(grp_name)//'/amr_'//TRIM(nchar)//'.out'//TRIM(ncharcpu)
     open(unit=10,file=nomfich,status='old',form='unformatted')
...
! ROM: comment the single follwing line for old stuff
     ...
     nomfich=TRIM(repository)//'/group_'//TRIM(grp_name)//'/hydro_'//TRIM(nchar)//'.out'//TRIM(ncharcpu)
     ...
     do ilevel=1,lmax
        ...
        if(ngrida>0)then
              ...
              do i=1,ngrida
                 ...
                 if(ok_cell)then
                    ...
                    if(    ix>=grid(ilevel)%imin.and.&
                       ...
                       grid(ilevel)%cube(ix,iy,iz,type)=var(i,ind,type)
                    endif
                 end if
              end do
           end do
           ...
           deallocate(xg,son,var,ref,x) !rho
        endif
     ...
     do iy=jmin,jmax
        ...
        do iz=kmin,kmax
           ...
           do ilevel=max(levelmin,lmin),lmax-1
              ...
              j=int(ymin*ndom)+1
              k=int(zmin*ndom)+1
              grid(lmax)%cube(ix,iy,iz,type)=grid(lmax)%cube(ix,iy,iz,type)+ &
                   & grid(ilevel)%cube(i,j,k,type)
           end do
        end do
     end do
  end do
end do
  !JL segfaulting here, I guess it's optional anyway ...
  ...
  !JL params need to default to -1
  !possibly allocate grid to the right size based on number of >0 index params
  
  do type=1,8


     ! get file name
     if (type.eq.rhog) then
        outfich_type="rho"
     else if (type.eq.rhod) then
        outfich_type="dust"
     else if (type.eq.temp) then
        outfich_type="temp"
     else if (type.eq.vx) then
        outfich_type="vx"
     else if (type.eq.vy) then
        outfich_type="vy"
     else if (type.eq.vz) then
        outfich_type="vz"
     else if (type.eq.metal) then
        outfich_type="Z"
     else if (type.eq.xhi) then
        outfich_type="xion"
     else
        cycle !didn't find a given index so skip
     end if
  ...
  if(TRIM(filetype).eq.'bin')then
     nomfich=TRIM(outfich)//TRIM(outfich_type)//'_'//TRIM(outnb)
     write(*,*)"Debug metal",type,outfich,metal
     ...
     if(nx_sample==0)then
        ...
        toto=grid(lmax)%cube(imin:imax,jmin:jmax,kmin:kmax,type)
        ...
        do i=1,nx_sample
           ...
           do j=1,ny_sample
              ...
              do k=1,nz_sample
                 ...
                 iz=min(iz,kmax)
                 izp1=min(iz+1,kmax)
                 toto(i,j,k)=dex*dey*dez*log10(grid(lmax)%cube(ix  ,iy  ,iz  , type))+ &
                      &      ddx*dey*dez*log10(grid(lmax)%cube(ixp1,iy  ,iz  , type))+ &
                      &      dex*ddy*dez*log10(grid(lmax)%cube(ix  ,iyp1,iz  , type))+ &
                      &      dex*dey*ddz*log10(grid(lmax)%cube(ix  ,iy  ,izp1, type))+ &
                      &      ddx*ddy*dez*log10(grid(lmax)%cube(ixp1,iyp1,iz  , type))+ &
                      &      dex*ddy*ddz*log10(grid(lmax)%cube(ix  ,iyp1,izp1, type))+ &
                      &      ddx*dey*ddz*log10(grid(lmax)%cube(ixp1,iy  ,izp1, type))+ &
                      &      ddx*ddy*ddz*log10(grid(lmax)%cube(ixp1,iyp1,izp1, type))
              end do
           end do
        end do
        toto=10.**(toto)
  ...
  if(TRIM(filetype).eq.'grafic')then
     ...
     do iz=kmin,kmax
        write(20)((real(grid(lmax)%cube(ix,iy,iz,type)),ix=imin,imax),iy=jmin,jmax)
     end do
     close(20)
  endif
  ...
  if(TRIM(filetype).eq.'vtk')then
     ...
     do k=kmin,kmax
        do j=jmin,jmax
           do i=imin,imax
              write(20,'(F9.3)')real((grid(lmax)%cube(i,j,k,type)))
           end do
        end do
     end do
     close(20)
